...
class WordChoice extends React.Component {
  ...
  handleChange(event) {
    ...
    majSaisieMot.then(() => {this.verificationEntree()})

  }

  /* Le pattern du formulaire ne fonctionne pas car la dernière 
   *  balise avant la balise de fermeture du form n'est pas un
   * '<input type="submit"/>'
   */
  verificationEntree() {
  ...
  render() {
    return (
      <div>
        <form>
          <label>
            Mot que vous souhaitez faire deviner à votre ami :
            <p></p>
            <input type="text" pattern='^[a-zA-Z]+$' placeholder='Écrivez ici votre mot' maxLength={30} value={this.state.value} onChange={this.handleChange} required />
          </label>
          <nav>
            <p></p>
